...
def double_exp_decay_fit(x_axis, y_axis):
    ...
    bounds = ([0, 0, 0, 0], [y_axis[0]+2, np.inf, y_axis[0]+2, np.inf])
    ...
    try: 
        popt, pcov = curve_fit(double_exp_decay, x_axis, y_axis, p0 = p0, bounds = bounds)
        ...
        print("A1 = {0}, t1 = {1}, A2 = {2}, t2 = {3}".format(*popt))
        #print the Beq and kobs values from the best fit

        perr = np.sqrt(np.diag(pcov))
        print("A1+- = {0}, t1+- = {1}, A2+- = {2}, t2+- = {3}".format(*perr))
        #print one standard deviation errors on the parameters

        plt.plot(x_axis, double_exp_decay(x_axis, *popt), 'r-', x_axis, y_axis, 'b-', label = 'fit')
        plt.show()
...
def double_exp_decay(x, A1, t1, A2, t2):
    return A1*np.exp(-x/t1) + A2*np.exp(-x/t2) + 0
...
for column in range(1, len(zeroed_data[0]), 2):   
    ...
    double_exp_decay_fit(x_axis, y_axis)
...
ws1 = wb.active
ws1.title = 'results'
double_exp_decay_header = ['column', 'A1', 't1', 'A2', 't2', 'A1+-', 't1+-', 'A2+-', 't2+-'] 

ws1.append(double_exp_decay_header)
