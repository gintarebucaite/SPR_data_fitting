...
raw_data = "betv4_h2_150s_association_250517.txt"
...
def kobs_fit(x_axis, y_axis):
    ...
    bounds = ([0, 0, 0, 0], [y_axis[-1]+10, np.inf, y_axis[-1]+10, np.inf])
    ...
    try: 
        ...
        popt, pcov = curve_fit(double_kobs, x_axis, y_axis, p0 = p0, bounds = bounds)
        ...
        print("Beq1 = {0}, kobs1 = {1}, Beq2 = {2}, kobs2 = {3}".format(*popt))
        perr = np.sqrt(np.diag(pcov))
        print("Beq1+- = {0}, kobs1+- = {1}, Beq2+- = {2}, kobs2+- = {3} ".format(*perr))

        result = [column, *popt, *perr]
        results.append(result)

        plt.plot(x_axis, double_kobs(x_axis, *popt), 'r-', x_axis, y_axis, 'b-', label = 'fit')
        plt.show()
...
def double_kobs(x, Beq1, kobs1, Beq2, kobs2):
    return (Beq1*(1-np.exp(-x*kobs1)))+(Beq2*(1-np.exp(-x*kobs2)))
...
for column in range(1, len(zeroed_data[0]), 2):   
    ...
    #when fitting the data, instead of 'function' type in either 'two_variables_fit' 
    #or 'four_variables_fit'
...
ws1 = wb.active
ws1.title = 'results'
double_kobs_header = ['column', 'Beq1', 'kobs1', 'Beq2', 'kobs2', 'Beq1+-', 
                      'kobs1+-', 'Beq2+-', 'kobs1+-' ]
ws1.append(double_kobs_header)
